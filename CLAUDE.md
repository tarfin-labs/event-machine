...
- **Guard semantics in parallel:** Regular `GuardBehavior` failure in a region = graceful failure — `TRANSITION_FAIL` recorded, machine stays in current state (same as non-parallel). `ValidationGuardBehavior` failure = "operation rejected" — entire parallel transition aborted, machine stays, `MachineValidationException` thrown (422 via endpoints). Dispatch mode: validation errors don't propagate (async).
...
- **Dispatch timing:** `ChildJobJob` / `ChildMachineJob` / `ChildMachineTimeoutJob` / queued `ListenerJob` are buffered in `MachineDefinition::$pendingChildDispatches` and dispatched by `Machine::persist()` — never mid-transition. These jobs restore the parent from the DB; dispatching before persist let them observe the pre-transition state (on sync queues the completion was silently discarded and the parent hung forever). Consequence: `QUEUE_CONNECTION=sync` runs full delegation chains inline and correctly — no queue worker needed for e2e tests.
