...
- **Path counts rise sharply — expect coverage percentages to drop.** On the three production machines this release was validated against, the totals went **239 → 878**, **99 → 390** and **223 → 862** paths (3.7–3.9×).
  ...
  Read those "before" figures precisely: they are **mid-release**, measured once the crash was fixed but before the final-state change. They are not what 9.16.0 reports, because 9.16.0 reports nothing for these three machines — `machine:paths` segfaults on all of them (exit 139). If you are coming from 9.16.0 with a machine that crashed, your before-figure does not exist; if you are coming from one that did not crash, measure it.

  Several changes contribute, and the largest is the final-state one: a final state was treated as terminal by both the enumerator and the scenario resolver, so any route continuing out of one was invisible — yet `findTransitionDefinition` walks the parent chain with no special case for `FINAL`, so a handler declared on an ancestor does fire from a final child, a compound `@done` whose guards all fail leaves the machine resting there, and a final state that delegates still has its `@done`/`@fail` routes. Any one of those three shapes is enough; a machine with none of them is unaffected by that change. The rest of the rise comes from a parallel state's own transitions, an `@always` declared on a parallel state, and the continuation beyond a region-declared escape — all previously dropped.

  How much a given machine moves therefore depends on which shapes it uses, and the range is wide: measured in isolation the final-state change alone ran from 1.0× (a machine with none of the three) to 12.5× across synthetic shapes, and a plain compound machine with no parallel state at all — a guarded compound `@done` plus a root-level handler — moved 2→8. **Do not read any single ratio as a rule** — measure your own machines with `machine:paths` before and after.
