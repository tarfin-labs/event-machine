...
**The reverse mismatch is a finding too.** An observed path matching nothing enumerated means a run took a route the analysis missed, so the enumeration is incomplete whatever the percentage says: `machine:coverage` warns and `--json` carries `unmatched_observed`, while `assertAllPathsCovered()` fails outright — in a suite those observations came from the same process, so they cannot be stale.
