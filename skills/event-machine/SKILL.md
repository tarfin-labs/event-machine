...
**Never read a truncated analysis as a complete one.** Enumeration is bounded by `--max-paths` (default 1000) and `--max-depth` (default 200). A branch cut at the depth ceiling is recorded as `TRUNCATED`; the path ceiling instead stops recording once the budget is spent, so those branches are absent and `path_limit_reached` is the only sign of them. The console names the ceiling that fired, and `--json` carries `analysis_truncated`, `path_limit_reached`, `depth_limit_reached` and `truncated_paths`. Check that flag before concluding a path does not exist — and note truncated paths are excluded from coverage accounting, so `machine:coverage` can read 100% over a partial analysis and reports `analysis_truncated` for exactly that reason.

**A final state is not where a path stops.** `findTransitionDefinition` walks the parent chain with no special case for `FINAL`, so the runtime leaves a final state three ways and all three are enumerated: a handler declared on an ancestor (the terminal path AND the continuation past the event — the machine really does rest there until it arrives), a compound `@done` whose branches are all guarded (same, because the guards can all fail), and a delegating final state (its `@done`/`@fail` still route). Only an **unguarded** compound `@done` replaces the terminal path, because then the machine never rests. Machines with any of these shapes enumerate far more paths than before this release, which lowers their coverage percentage — the earlier figure was computed over a list that was missing real routes.

**The reverse mismatch is a finding too.** An observed path matching nothing enumerated means a run took a route the analysis missed, so the enumeration is incomplete whatever the percentage says: `machine:coverage` warns and `--json` carries `unmatched_observed`. The assertions deliberately do NOT fail on it — the tracker can produce a signature no enumerated path matches for reasons of its own (a parallel machine records a region leaf where enumeration records the container), so gating on it would fail suites whose analysis is complete. Read it as a hint, not a verdict.
...
8. **`--path=N` is not a durable selector** — `machine:scenario` now lists routes cheapest first by a `weight` shown on each stats line, so a pinned index selects a different route than it did before this release. Worse, routes tied on *both* weight and step count have **no promised order** at all: it is stable for a given machine but nothing guarantees which comes first. Pick a route by reading its signature, not by remembering an index. The weight measures what the scenario must supply that the runtime cannot — an event to send, a child outcome to stand in for, guards to pin across regions — and is deliberately **not** a count of the lines the scaffold will contain: a transient state emits a guard block and still weighs 0, a delegation state emits one pre-filled line and weighs 3.
...
| `NoScenarioPathFoundException` | The search can't reach target from source — **or** the search hit its iteration ceiling first (the `truncated()` variant, whose message says "truncated at the search limit"). Raised by `ScenarioPathResolver::resolve()`, whose only caller is `ScenarioValidator::validatePaths()` — it converts both variants to error strings, and the scaffold command never triggers it at all. You see the exception only when calling `resolve()` yourself | Read which of the two it is. Genuinely unreachable: run `machine:paths` to find actual paths, add guard overrides for branching. Truncated: raise `--max-iterations` |
...
2. **Scaffold with `machine:scenario`** — accept the cheapest route it picks as a starting point, and check the listing's `weight` column if you meant a different one.
