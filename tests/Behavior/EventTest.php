...
test('interact with data', function (): void {
    $payload = [
        'name'      => 'John',
        'age'       => '30',
    ...
    expect($event->integer('age'))->toBeInt()->toEqual(30)
        ->and($event->str('name'))->toEqual(str('John'))
        ->and($event->date('birthdate'))->toEqual(Date::parse('1994-01-01'))
        ->and($event->collection('tags'))->toEqual(collect());
});
