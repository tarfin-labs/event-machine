...
it('full lifecycle: create → dispatch → jobs complete → onDone → next state', function (): void {
    ...
    expect($machine->state->context->get('regionAData'))->toBeNull();
    expect($machine->state->context->get('regionBData'))->toBeNull();
...
it('sequential fallback when dispatch disabled', function (): void {
    ...
    expect($machine->state->context->get('regionAData'))->toBe('processed_by_a');
    expect($machine->state->context->get('regionBData'))->toBe('processed_by_b');
...
it('context merge preserves keys set by first job when second job runs', function (): void {
    ...
    expect($afterA->state->context->get('regionAData'))->toBe('processed_by_a');
    expect($afterA->state->context->get('regionBData'))->toBeNull();
    ...
    expect($afterBoth->state->context->get('regionAData'))->toBe('processed_by_a');
    expect($afterBoth->state->context->get('regionBData'))->toBe('processed_by_b');
});
