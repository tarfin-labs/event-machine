...
5. **`machine:scenario`'s operator-facing text changes three times** — the stats line gains a trailing
   weight (§6 item 5); both commands' `--max-iterations` option description is reworded, since it called
   the search BFS, which `TruncationVocabularyTest` bans across both command files (§6 item 6's own list
   is `hop`/`breadth`/`shortest` over the four render sites, so it is the test that forces this one); and
   a single-path resolution
   now prints a listing where it previously printed nothing at all, since §6 item 5 requires the weight
   *including when only one path is found*. §6 item 6's four truncation *messages* need no reword — none
   of them ever carried the banned words — which is why the second change lands on the option text.
