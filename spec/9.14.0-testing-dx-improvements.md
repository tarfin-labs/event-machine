...
4. **`assertBehaviorRan()` accepts `string|array`** — array elements accept the same union the scalar form accepts today (behavior FQCN or inline-behavior string key, freely mixed). When given an array, asserts each entry ran in order with standard fail-fast semantics; the failure message names the first entry that did not run. An empty array throws `InvalidArgumentException`.
