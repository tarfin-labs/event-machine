...
| D4 | **`handleParallel()` is over 330 lines and carries around eleven concerns** — group reuse, region sub-enumeration, budget arithmetic, nested-group merging, truncation propagation, escape collection, escape following, `@done`, `@fail`, own-transition partitioning, and the dead-end/deferred decision. The obvious first extraction is the `@done`/`@fail` pair, two ~36-line near-clones differing only in the property and the event literal. | **Commitment honoured in 9.19.0.** The `@done`/`@fail` pair is now one `followParallelOutcome()` taking the transition and the event literal. Measured with a cognitive-complexity pass over all 5,957 functions in the repo: `handleParallel` went from **COG 80 / 351 lines to 55 / 286**, and the extracted method is COG 10 / 51. It also moved from 3rd to 5th most complex function in `src`. Worth recording why it was nearly deferred again: the same measurement showed two functions are *worse* — `ScenarioPathResolver::getNextStates` (94) and `MachineRouter::register` (81) — and "something else is worse" is exactly the self-renewing excuse this row was written to defeat. The original reason, kept because it is still true of the rest of the method: each audit round touched a *different* arm, so a rewrite is the one change the suite cannot validate by diff. |
