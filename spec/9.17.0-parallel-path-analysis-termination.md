...
| E4 | Every transition the runtime can take is represented somewhere in the output, or the analysis reports that it was truncated. Silent omission is a defect. Truncation is an exceptional outcome, not a way to satisfy E4: the reproduction stub and every machine in the existing test suites enumerate to completion without it. **One known exception, unfixed: a targetless self-transition is dropped with no path and no truncation flag — see §12 D12.** It is pre-existing on `main`, not a regression of this change, but E4 as written does not hold in that one shape. |
| E5 | No enumerated path is classified as a condition the runtime does not exhibit. In particular, a state the runtime can leave is never classified as a dead end. **The dead-end half holds; the general half has a known exception — see §12 D10**, where a transition declared on a parallel state targeting its own region yields a region `LOOP` the runtime does not exhibit, because the runtime exits the whole parallel instead. |
...
| E3/E4 change what is enumerated for parallel machines | path counts, path **types**, region path counts and coverage percentages move — in a minimal shape only the type moves, `dead_end` → `happy`, while the machine path count stays at 1 and the region count goes 5 → 2 — for every parallel machine whose parallel state **or an ancestor** carries a transition — the T5 exception. Region enumeration escapes through inherited ancestor transitions today, which is §2.1's premise, so the radius is wider than "parallel states carrying their own transitions" (an earlier revision) but narrower than "every parallel machine" (the revision after it): with no transition at or above the parallel state, output is byte-identical |
| S2 adds a step to routes through parallel regions | those routes did not resolve at all before. **But it also changes the answer for targets that already resolved**, which an earlier revision of this row missed: the same descent adds routes passing *through* a region on the way to a target outside it, so `resolveAll()` returns more paths for an unchanged query. Measured on `ScenarioTestMachine`, `reviewing --START_PARALLEL--> skipped` goes **1 → 3**. `machine:scenario` then prints "Found 3 paths" and offers a `--path=N` chooser where it used to scaffold silently, so **`--path` indices are not stable across this release** — a script pinned to `--path=0` may scaffold a different route |
...
| D5 | **`resolveClassFromFile()` is triplicated** across `MachineCoverageCommand`, `MachinePathsCommand` and `ExportXStateCommand`, byte-identical but for two comment lines — and it is not merely duplication. It `require_once`s the file, so it loads code into the process, and its `preg_match('/class\s+(\w+)\s+extends/')` takes the **first** extending class in the file: a file declaring a helper class before the machine resolves to the wrong FQCN, which the command then calls `::definition()` on. | **No longer deferred — closed in round 10.** It was deferred as scope, then a security probe showed the hazard was not only a wrong answer but four uncaught throwables: a **directory** argument (`file_exists` is true for one, and reading it raises `Is a directory`), an unreadable file, an unparseable file, and a file that throws at load. The three copies are now one `protected` method on `ResolvesMachineDefinition`, hardened with `is_file`, `is_readable`, and a `try/catch` around the load — both the `require_once` and the `class_exists()` that autoloads. The first-extending-class regex remains, and remains the wrong answer for a file that declares a helper class above the machine, but it now fails cleanly; parsing instead of matching is the real fix and is not this release's. |
| D6 | **The `1000`/`200` ceilings are literals in six declaration sites** rather than `config/machine.php`, unlike `max_transition_depth`: `PathEnumerator`'s constructor, `MachineDefinition::enumeratePaths()`, both coverage assertions on `Machine`, and the `machine:paths` and `machine:coverage` signatures. (The scenario search ceiling adds five more `1000`s under a different name — `ScenarioPathResolver:37`, `ScenarioValidator:32`, `MachineScenarioValidateCommand:19` and `:24`, and `MachineScenarioCommand:27` — which is why a plain grep for the literal gives a larger number. An earlier revision of this parenthetical said three and omitted `machine:scenario` entirely.) | Each is overridable at the point it matters; config-backing is ergonomics, not correctness. |
| D7 | **The two `catch (\Throwable) {}` reload fallbacks in `Machine`** — in `send()` and in `dispatchPendingChildJobs()` — state the fallback but not why the caller must not see the failure, and catch programming errors alongside transient ones. | The behaviour is pre-existing and unchanged on this branch; the comments explaining it are new (both blocks gained six lines here), so "unchanged" is true of the code and not of the text. Note the reason is weaker than it looks for `send()`: continuing on a stale local state then mutates a forked timeline under a freshly acquired lock, which does look like success to the caller. |
...
| D12 | **A targetless self-transition is dropped without a trace.** `on PING => ['actions' => 'bumpAction']` is a transition the runtime takes — it runs the branch actions and stays — but every follow site skips it with a bare `continue` on `!$branch->target instanceof StateDefinition` (`PathEnumerator.php:1125-1129` and the same test in `handleFinal`/`enumerateMachineInvoke`). No path, no truncation flag: a machine with such a handler plus `DONE => finished` enumerates only `[happy] idle→[DONE]→finished` with `analysisTruncated=false`. | Deferred, and pre-existing on main (`main:PathEnumerator.php:463-466` has the same `continue`), so it is not a regression of this release. Recording it needs a decision this change does not make: a self-transition does not extend the state sequence, so it either needs a path type of its own or it changes what a path signature means — and the tracker would have to agree, or every such machine starts reporting unmatched observations. Found after the sweep that produced the rows above, and checked against main separately. |
| D13 | **A final state that delegates still records a terminal path.** `handleFinal()` skips the terminal record when the enclosing compound's `@done` has an unguarded branch — the machine never rests there, so recording a rest is wrong. The same argument looks like it should apply to a final state carrying a `machine`/`job` invoke with an unguarded `@done`. | Not applied, because the two are not the same shape. A compound `@done` fires when the region completes and has no alternative; an invoke can also fail or time out, so a final state with an unguarded `@done` but no `@fail` genuinely does rest there when the child fails. The correct rule needs the invoke's outcomes to be exhaustive, which means resolving what the runtime does with a failing child and no `@fail` handler — a runtime question, not an analysis one, and unanswered here. |
