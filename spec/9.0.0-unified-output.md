...
The output method (`$machine->result()` deleted). Returns only the **output content** (not the envelope — that's an HTTP concern).
...
Base class for output behaviors (`ResultBehavior` deleted). Same parameter injection system:
...
| `ResultBehavior` | Deleted — `OutputBehavior` is the only class |
| `$machine->result()` | Deleted — `$machine->output()` is the only method |
...
| Step | Change | Files |
|------|--------|-------|
| 1 | Add `OutputBehavior` base class | `src/Behavior/OutputBehavior.php` |
...
| 13 | Delete `ResultBehavior` class entirely | `src/Behavior/ResultBehavior.php` (deleted) |
| 14 | Delete `$machine->result()` method | `src/Actor/Machine.php` |
...
| 19 | Delete `ChildMachineDoneEvent::result()` method | `src/Behavior/ChildMachineDoneEvent.php` |
...
| Step | File | Changes | Detail |
|------|------|---------|--------|
| 3 | `docs/advanced/delegation-data-flow.md` | ~15 | `ChildMachineDoneEvent::result()` removed, `ResultBehavior` → `OutputBehavior`, `contextKeys` in forward config → `output`, `PaymentStepResult` → `PaymentStepOutput` |
...
| 5 | `docs/advanced/machine-delegation.md` | ~10 | `ChildMachineDoneEvent` accessor docs — `result()` removed, `Machine::fake(output:)` |
...
| **ChildMachineDoneEvent** | Yes | `result()` accessor deleted — `output()` is the only accessor |
...
- `tests/Features/OutputBehaviorTriggeringEventTest.php` — 6 tests, all use `'output'` on states + `->output()` calls
- `tests/Features/OutputBehaviorInjectionTest.php` — 10 tests, all use `behavior['outputs']` + `'output'` on states
...
```
- simulateChildDone accepts $output param
- simulateChildDone populates ChildMachineDoneEvent with output field
- simulateChildFail passes $output correctly
- Machine::fake(output:) works correctly
```
